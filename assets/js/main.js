...
var $headerContent = $('. style-header > *');
...
function bodyGetScrollbarWidth() {
    ...
    scrollDiv.className = ' style-body-scrollbar-measure';
...
function initNavbar() {
    var self = this;
    var $navbarTop = (0, _utility.$)('. style-navbar-top');
    ...
    if ($mobileNavItems.length) {
        $mobileNavItems.each(function () {
            ...
            $mobileNav.find('. style-navbar-mobile-content > ul. style-nav').append($nav);
        });

        var $nav = (0, _utility.$)('. style-navbar-mobile-content > ul. style-nav');

        // remove background images
        $nav.find('.bg-image, .bg-video').remove();

        // remove mega menus
        $nav.find('. style-mega-item > .dropdown').each(function () {
    ...
    function onScrollNav() {
        ...
        if (stickyOn) {
            $navbarTop.addClass(' style-navbar-fixed');
            $navbarFake.show();
        } else {
            $navbarTop.removeClass(' style-navbar-fixed');
            $navbarFake.hide();
        }
    }
    if ($navbarTop.hasClass(' style-navbar-sticky')) {
    ...
    function correctDropdown($item) {
        if ($item.parent().is('. style-nav')) {
            var $dropdown = $item.children('.dropdown');
            var $parent = $item.closest('. style-navbar');
    ...
    function closeSubmenu($item) {
        if ($item.length) {
            ...
            _utility.$wnd.trigger(' style-closed-submenu', [$item]);
        }
    }
    function openSubmenu($item) {
        if (!$item.hasClass('open')) {
            ...
            $item.addClass('open');
            _utility.$wnd.trigger(' style-opened-submenu', [$item]);
        }
    }
    var dropdownTimeout = void 0;
    $navbarTop.on('mouseenter', 'li. style-drop-item', function () {
        var $item = (0, _utility.$)(this);
        var $openedSiblings = $item.siblings('.open').add($item.siblings().find('.open')).add($item.parents('. style-nav:eq(0)').siblings().find('.open')).add($item.parents('. style-nav:eq(0)').siblings('.open')).add($item.parents('. style-nav:eq(0)').parent().siblings().find('.open'));

        clearTimeout(dropdownTimeout);
        closeSubmenu($openedSiblings);
        openSubmenu($item);
    }).on('mouseleave', 'li. style-drop-item', function () {
        var $item = (0, _utility.$)(this);
        clearTimeout(dropdownTimeout);
    ...
    var $autohideNav = $navbarTop.filter('. style-navbar-autohide');
    self.throttleScroll(function (type, scroll) {
        var start = 400;
        var hideClass = ' style-onscroll-hide';
        var showClass = ' style-onscroll-show';
        ...
        // add solid color
        if ($navbarTop.hasClass(' style-navbar-transparent')) {
            $navbarTop[(scroll > 70 ? 'add' : 'remove') + 'Class'](' style-navbar-solid');
        }
    });
}
...
function initNavbarSide() {
    var self = this;
    var $overlay = (0, _utility.$)('<div class=" style-navbar-overlay">').appendTo(_utility.$body);

    // side navbars
    var $leftSide = (0, _utility.$)('. style-navbar-left-side');
    var $rightSide = (0, _utility.$)('. style-navbar-right-side');
    var $sideNavs = (0, _utility.$)('. style-navbar-side');
    ...
    self.openSide = function ($side, speed) {
        ...
        _utility.tween.to($side, speed || 0.4, {
            x: $side.hasClass(' style-navbar-left-side') ? '100%' : '-100%',
            force3D: true
        });

        // show overlay
        if ($side.hasClass(' style-navbar-overlay-content')) {
    ...
    self.closeSide = function ($side, speed) {
        ...
        if (!$sideNavs.filter('. style-navbar-overlay-content.open').length) {
            // hide overlay
            _utility.tween.to($overlay, 0.3, {
                opacity: 0,
                display: 'none',
    ...
    // overlay
    _utility.$doc.on('click', '. style-navbar-overlay', function () {
...
function initNavbarDropEffect1() {
    var self = this;
    var $navbars = (0, _utility.$)('. style-navbar-side, . style-navbar-full');
    ...
    function updateSideNavDropdown($item) {
        var $nav = $item.parents('. style-navbar:eq(0)');
        var $khNav = $nav.find('. style-nav');
        var $nanoCont = $khNav.children('.nano-content');
        var $khNavRow = $khNav.parent();
        var $drop = $nav.find('. style-drop-item.open > .dropdown:not(.closed)');

        if ($drop.length) {
            var dropHeight = $drop.innerHeight();

            // vertical center for dropdown
            if ($khNavRow.hasClass(' style-nav-row-center')) {
                $drop.css({
                    top: 0
                });
    ...
    $navbars.on('click', '. style-drop-item > a', function (e) {
...
function initBackgrounds() {
    ...
    var $backgrounds = (0, _utility.$)('. style-page-background-top, . style-page-background-bottom, . style-page-background-fixed');
...
function initCounters() {
    var self = this;
    var $progressCount = (0, _utility.$)('. style-progress. style-count');
    var $numberCount = (0, _utility.$)('. style-count:not(. style-progress)');

    // set default progress
    $progressCount.each(function () {
        (0, _utility.$)(this).attr('data- style-count', (0, _utility.$)(this).attr('data-progress')).attr('data- style-mask', (0, _utility.$)(this).attr('data-progress-mask')).find('. style-progress-line > div').css('width', ((0, _utility.$)(this).attr('data- style-count-from') || '0') + '%').find('. style-progress-percent').html('');
    });

    // set default numbers
    $numberCount.each(function () {
        (0, _utility.$)(this).attr('data- style-count', (0, _utility.$)(this).attr('data- style-count') || parseInt((0, _utility.$)(this).text(), 10)).html((0, _utility.$)(this).attr('data- style-count-from') || '0');
    });
    ...
    function runCounters() {
        ...
        var progress = $progressCount.filter('[data- style-count]');
        var numbers = $numberCount.filter('[data- style-count]');
        countersNum = progress.length + numbers.length;

        // progress
        $progressCount.filter('[data- style-count]').each(function () {
            var $item = (0, _utility.$)(this);
            if (self.isInViewport($item)) {
                var count = {
                    curr: $item.attr('data- style-count-from') || '0',
                    to: $item.attr('data- style-count'),
                    mask: $item.attr('data- style-mask') || '{$}%'
                };
                var $itemLine = $item.find('. style-progress-line > div');
                var $itemLabel = $item.find('. style-progress-percent');
                ...
                $item.removeAttr('data- style-count');
            }
        });

        // number
        $numberCount.filter('[data- style-count]').each(function () {
            var $item = (0, _utility.$)(this);
            if (self.isInViewport($item)) {
                var count = {
                    curr: $item.text(),
                ...
                $item.removeAttr('data- style-count data- style-count-from');
                _utility.tween.to(count, 1, {
                    curr: count.to,
                    roundProps: 'curr',
                    ease: Circ.easeIn,
...
function initStore() {
    ...
    // scroll to ratings
    _utility.$doc.on('click', 'a. style-product-rating', function (e) {
        var isHash = this.hash;
        if (isHash) {
            var $hashBlock = (0, _utility.$)(isHash).parents('. style-tabs:eq(0)');
            if ($hashBlock.length) {
                self.scrollTo($hashBlock);
            }
            (0, _utility.$)('. style-tabs').find('[data-toggle="tab"][href="' + isHash + '"]').click();
        }
        e.preventDefault();
    });
}
...
function initNewsBox() {
    _utility.$doc.on('click', '. style-news-box . style-news-box-item', function () {
        var $this = (0, _utility.$)(this);
        var $info = $this.parents('. style-news-box:eq(0)').find('. style-news-box-each-info');

        // get data
        var data = {
            title: $this.find('. style-news-box-item-title').html(),
            img: $this.find('. style-news-box-item-full-img').attr('src'),
            img_alt: $this.find('. style-news-box-item-full-img').attr('alt'),
            categories: $this.find('. style-news-box-item-categories').html(),
            text: $this.find('. style-news-box-item-text').html(),
            url: $this.find('. style-news-box-item-url').attr('href'),
            date: $this.find('. style-news-box-item-date').html()
        };

        // set data
        $info.find('. style-news-box-item-title').html(data.title);
        if ($info.find('. style-news-box-item-image > img').length) {
            $info.find('. style-news-box-item-image > img').attr('src', data.img).attr('alt', data.img_alt);
        } else {
            $info.find('. style-news-box-item-image').css('background-image', 'url("' + data.img + '")');
        }
        $info.find('. style-news-box-item-categories').html(data.categories);
        $info.find('. style-news-box-item-text').html(data.text);
        $info.find('. style-news-box-item-more').attr('href', data.url);
        $info.find('. style-news-box-item-date').html(data.date);

        // activate item
        $this.addClass(' style-news-box-item-active').siblings().removeClass(' style-news-box-item-active');
    });

    // click on active item on load
    (0, _utility.$)('. style-news-box . style-news-box-item-active').trigger('click');
}
...
function initAnchors() {
    ...
    // click on anchors
    var $leftSideNav = (0, _utility.$)('. style-navbar-left-side');
    var $rightSideNav = (0, _utility.$)('. style-navbar-right-side');
    function closeNavs() {
        self.closeSide($leftSideNav);
        self.closeSide($rightSideNav);
        self.closeFullscreenNavbar();
    }
    _utility.$doc.on('click', '.navbar a, . style-navbar a, a.btn, a. style-btn, a. style-anchor', function (e) {
        var isHash = this.hash;
    ...
    var $anchorItems = (0, _utility.$)('. style-navbar . style-nav > li > a[href*="#"]');
    var anchorBlocks = [];
...
function initVideoBlocks() {
    ...
    function addPlainPlayButton($plainCont) {
        $plainCont.find('. style-video-plain-toggle').html(self.options.templates.plainVideoIcon);
    }
    function addPlainLoadButton($plainCont) {
        $plainCont.find('. style-video-plain-toggle').html(self.options.templates.plainVideoLoadIcon);
    }
    (0, _utility.$)('. style-plain-video[data-video]').each(function () {
        ...
        if (api && api.isValid()) {
            ...
            // add play button
            $plainCont.append('<span class=" style-video-plain-toggle"></span>');
...
function initGIF() {
    ...
    function playGif(item) {
        ...
        if (!item.gifPlaying) {
            ...
            if (item.khGifLoaded) {
                $item.addClass(' style-gif-playing');
                $item.find('img').attr('src', $item.find('img').attr('data-gif'));
            } else if (!item.khGifLoading) {
                item.khGifLoading = 1;
                $item.addClass(' style-gif-loading');
                loadGif($item.find('img').attr('data-gif'), function () {
                    item.khGifLoaded = 1;
                    $item.removeClass(' style-gif-loading');
                    if (item.gifPlaying) {
    ...
    function stopGif(item) {
        ...
        if (item.gifPlaying) {
            item.gifPlaying = false;
            $item.removeClass(' style-gif-playing');
            $item.find('img').attr('src', $item.find('img').attr('data-gif-static'));
        }
    }

    // prepare gif containers
    (0, _utility.$)('. style-gif').each(function () {
        var $this = (0, _utility.$)(this);
        // add toggle button
        $this.append('<a class=" style-gif-toggle">' + self.options.templates.gifIcon + '</a>');

        // add loading circle
        $this.append('<div class=" style-loading-spinner"><i></i></div>');

        $this.find('img').attr('data-gif-static', $this.find('img').attr('src'));
    });

    // hover gif
    (0, _utility.$)('. style-gif-hover').on('mouseenter', function () {
    ...
    // click gif
    (0, _utility.$)('. style-gif-click').on('click', function () {
        if (this.gifPlaying) {
            (0, _utility.$)(this).removeClass('hover');
            stopGif(this);
        } else {
    ...
    var $gifVP = (0, _utility.$)('. style-gif-viewport');
    if ($gifVP.length) {
    ...
    // autoplay gif
    (0, _utility.$)('. style-gif:not(. style-gif-click):not(. style-gif-hover):not(. style-gif-viewport)').each(function () {
        playGif(this);
    });
}
...
function initInfoBoxes() {
    ...
    // close
    _utility.$doc.on('click', '. style-info-box . style-info-box-close', function (e) {
        e.preventDefault();
        var $box = (0, _utility.$)(this).parents('. style-info-box:eq(0)');
...
function initForms() {
    ...
    (0, _utility.$)('form:not(. style-form-ajax):not(. style-mchimp):not([novalidate])').each(function () {
    ...
    // ajax form
    (0, _utility.$)('form. style-form-ajax:not([novalidate])').each(function () {
        (0, _utility.$)(this).validate({
            errorClass: ' style-error',
            errorElement: 'div',
            errorPlacement: function errorPlacement(error, element) {
            ...
            submitHandler: function submitHandler(form) {
                var $responseSuccess = (0, _utility.$)(form).find('. style-form-response-success');
                var $responseError = (0, _utility.$)(form).find('. style-form-response-error');
...
function initFormsMailChimp() {
    var $mchimp = (0, _utility.$)('form. style-mchimp');
    ...
    $mchimp.each(function () {
        ...
        var validator = $form.validate({
            ...
            submitHandler: function submitHandler() {
                var $responseSuccess = $form.find('. style-form-response-success');
                var $responseError = $form.find('. style-form-response-error');
...
function initAudioPlayer() {
    ...
    /* Plain audio players */
    var $playersPlain = (0, _utility.$)('. style-audio-plain');
    ...
    var PlayersPlain = function PlayersPlain($item) {
        ...
        self.$playPauseBtn = $item.find('. style-audio-plain-play-pause');
        self.$progress = $item.find('. style-audio-progress-current');

        self.$timer = $item.find('. style-audio-plain-duration');
        self.$timer.attr('data-duration', self.$timer.text());

        function onPlay() {
            $item.addClass(' style-audio-plain-playing');
        }
        function onStop() {
            self.seek(0);
            self.step();
            self.$item.removeClass(' style-audio-plain-playing');
    ...
    if (typeof Hammer !== 'undefined') {
        var $progresses = $playersPlain.find('. style-audio-progress');
        $progresses.each(function () {
            ...
            HammerProgress.on('pan press pressup', function (e) {
                ...
                if (e.isFinal || e.type === 'pressup') {
                    if (!curApi) {
                        curApi = $curProgressCont.parents('. style-audio-player-main, . style-audio-plain')[0].audioAPI;
                    }
                    if (curApi) {
...
function initImageSlider() {
    var $sliders = (0, _utility.$)('. style-image-slider');
    ...
    function selectSlide($slider) {
        ...
        busy = 1;
        var data = $slider.data(' style-image-slider');

        // get next slide
        if (slideNum === false) {
            slideNum = data.$thumbs.find('. style-image-slider-thumbs-active').index() + 1;
        }
    ...
    $sliders.each(function () {
        ...
        // parse all slides
        $this.find('. style-image-slider-item').each(function () {
            var $slide = (0, _utility.$)(this);
            slides.push({
                image: $slide.find('. style-image-slider-img').attr('src'),
                thumb: $slide.find('. style-image-slider-img').attr('data-thumb'),
                content: $slide.find('. style-image-slider-content').html() || ''
            });
        });
        ...
        slides.forEach(function (item, k) {
            thumbs += '<li class="' + (k === defaultSlide ? ' style-image-slider-thumbs-active' : '') + '" style="background-image: url(\'' + item.thumb + '\');"><div class=" style-image-slider-thumbs-overlay"></div></li>';
        });
        var template = '\n            <div class=" style-image-slider-bg" style="background-image: url(\'' + slides[defaultSlide].image + '\');"></div>\n            <div class=" style-image-slider-bg-transition"></div>\n            <div class=" style-image-slider-content" style="' + (slides[defaultSlide].content ? '' : 'opacity: 0;') + '">\n                <div class="nano">\n                    <div class="nano-content">' + slides[defaultSlide].content + '</div>\n                </div>\n            </div>\n            <div class=" style-image-slider-thumbs">\n                <ul>' + thumbs + '</ul>\n            </div>\n        ';

        // append template in slider
        $this.append(template);

        // move thumbs cont
        var $thumbs = $this.find('. style-image-slider-thumbs');
        ...
        function selectThumb(i) {
            $thumbs.find('li:eq(' + i + ')').addClass(' style-image-slider-thumbs-active').siblings().removeClass(' style-image-slider-thumbs-active');
        ...
        function stopAutoplay() {
            ...
            if (!dontTouchCount) {
                $thumbs.find('. style-image-slider-thumbs-count').remove();
            }
        }
        function runAutoplay() {
            ...
            var $currentThumb = $thumbs.find('. style-image-slider-thumbs-active');
            ...
            // add new timer
            var $timer = (0, _utility.$)('<div class=" style-image-slider-thumbs-count"></div>').text(convertTime(autoplay));
        ...
        var data = {
            ...
            $content: $this.find('. style-image-slider-content .nano-content'),
            $contentWrapper: $this.find('. style-image-slider-content'),
            $bg: $this.find('. style-image-slider-bg'),
            $bgTransition: $this.find('. style-image-slider-bg-transition'),
        ...
        $this.data(' style-image-slider', data);
    ...
    // click handler
    _utility.$doc.on('click', '. style-image-slider . style-image-slider-thumbs li:not(. style-image-slider-thumbs-active)', function () {
        var $li = (0, _utility.$)(this);
        var $slider = $li.parents('. style-image-slider:eq(0)');
        selectSlide($slider, $li.index());
    });

    // pause autoplay on mouseenter
    _utility.$doc.on('mouseenter', '. style-image-slider', function () {
        var data = (0, _utility.$)(this).data(' style-image-slider');
        if (data) {
            data.pauseAutoplay();
        }
    });
    _utility.$doc.on('mouseleave', '. style-image-slider', function () {
        var data = (0, _utility.$)(this).data(' style-image-slider');
        if (data) {
            data.resumeAutoplay();
        }
...
function initInstagram() {
    var self = this;
    var $instagram = (0, _utility.$)('. style-instagram');
...
function initTwitter() {
    var self = this;
    var $twtFeeds = (0, _utility.$)('. style-twitter-list');
...
function initPluginStickySidebar() {
    ...
    (0, _utility.$)('. style-sidebar-sticky').each(function () {
        var $this = (0, _utility.$)(this);
        var $parent = $this.parent();

        $parent.addClass(' style-sidebar-sticky-parent');
...
function initPluginFlickity() {
    ...
    function addDefaultArrows($carousel) {
        (0, _utility.$)('<div class=" style-flickity-arrow  style-flickity-arrow-prev"><span class="ion-ios-arrow-back"></span></div>').on('click', function () {
            $carousel.flickity('previous');
        }).appendTo($carousel);

        (0, _utility.$)('<div class=" style-flickity-arrow  style-flickity-arrow-next"><span class="ion-ios-arrow-forward"></span></div>').on('click', function () {
    ...
    // carousel
    (0, _utility.$)('. style-carousel > . style-carousel-inner').each(function () {
...
function initPluginPhotoswipe() {
    var $gallery = (0, _utility.$)('. style-popup-gallery');
    ...
    function parseThumbnailElements(el) {
        var thumbElements = (0, _utility.$)(el).find('a. style-gallery-item');
        var items = [];
        ...
        thumbElements.each(function eachThumbs() {
            descrElement = (0, _utility.$)(this).next('. style-gallery-item-description');
    ...
    $gallery.each(function eachGallery() {
        ...
        $thisGallery.on('click', 'a. style-gallery-item', function onGalleryItemClick(e) {
            e.preventDefault();
            var index = 0;
            var clicked = this;
            $thisGallery.find('a. style-gallery-item').each(function eachGalleryItem(idx) {
                if (this === clicked) {
                    index = idx;
                    return false;
                }
...
function initPluginCountdown() {
    ...
    (0, _utility.$)('. style-countdown').each(function () {
...
function initPluginSeiyriaBootstrapSlider() {
    ...
    (0, _utility.$)('. style-input-slider').each(function () {
        var $this = (0, _utility.$)(this);
        var $input = $this.find('input');
        var $labels = [];

        for (var k = 0; k < 3; k++) {
            $labels.push($this.find('. style-input-slider-value-' + k));
        }
...
function initPluginSummernote() {
    ...
    (0, _utility.$)('. style-summernote').summernote({
        height: 300,
        dialogsInBody: true,
        toolbar: [
